...
def wav2mfc(PATH, files):
    ...
    mfc_audio_list = [(PATH + (re.sub('.raw', '-n.mfc', files[i]))) for i in range(len(files))]
    wav_audio_list = [(PATH + (re.sub('.raw', '-n.wav', files[z])) + ' ') for z in range(len(files))]
    hcopy = 'HCopy -C '
    config_file = "/home/ferreiraa/Mestrado-PC/HTK_features/HTK-Tool/config "
