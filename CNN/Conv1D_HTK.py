...
for i in range(len(train_audio_paths)):
    train_audio_paths_mfc.append(
        re.sub(r'\.wav', '.mfc.csv', train_audio_paths[i]))

# Create 2 datasets, one for training and the other for validation
train_ds = HTK_paths_and_labels_to_dataset(
    train_audio_paths[:5], train_labels[:5], NORM_TRAIN)
